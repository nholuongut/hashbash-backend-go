...
hashbash-cli: $(SRC_FILES)
	go build -ldflags "-X main.version=$(shell git describe)" -o hashbash-cli github.com/nholuongut/hashbash-backend-go/cmd/hashbash-cli

hashbash-engine: $(SRC_FILES)
	go build -ldflags "-X main.version=$(shell git describe)" -o hashbash-engine github.com/nholuongut/hashbash-backend-go/cmd/hashbash-engine

hashbash-webapp: $(SRC_FILES)
	go build -ldflags "-X main.buildTimestamp=$(shell date --utc --iso-8601=seconds) -X main.gitRevision=$(shell git rev-parse HEAD) -X main.version=$(shell git describe)" -o hashbash-webapp github.com/nholuongut/hashbash-backend-go/cmd/hashbash-webapp
